...
public class LifeListPrimitive  extends AbstractLife {
    ...
    protected class CellRow extends IntArrayList{
        protected int index;
    ...
    public LifeListPrimitive(int canvasHeight, int canvasWidth) {
    ...
    private CellRow nextRow(Integer col, CellRow lRow, CellRow cRow, CellRow rCol){
        ...
        IntListIterator centerIt;  //center column iterator
        IntListIterator leftIt;    //left column iterator
        IntListIterator rightIt;   //right column iterator
        ...
        int nextL = Integer.MAX_VALUE;        //next existing left cell
        int nextC = Integer.MAX_VALUE;        //next existing center cell
        int nextR = Integer.MAX_VALUE;        //next existing right cell
        ...
        if (leftRowExists){
            ...
            lIndex = leftIt.nextInt() - 1; //actual index is left of first cell
            nextL =  leftIt.hasNext() ? leftIt.nextInt() : Integer.MAX_VALUE;
        } else {
            leftIt = IntIterators.EMPTY_ITERATOR;
            lIndex = Integer.MAX_VALUE; //important to work with min search
        }
        if (centerRowExists){
            centerIt = cRow.iterator();
            centerBuffer = 1; //set first bit
            cIndex = centerIt.nextInt() - 1; //actual index is left of first cell
            nextC = centerIt.hasNext() ? centerIt.nextInt() : Integer.MAX_VALUE;
        } else {
            centerIt = IntIterators.EMPTY_ITERATOR;
            cIndex = Integer.MAX_VALUE; //important to work with min search
        }
        if(rightRowExists){
            rightIt = rCol.iterator();
            rightBuffer = 1; //set first bit
            rIndex = rightIt.nextInt() - 1; //actual index is left of first cell
            nextR = rightIt.hasNext() ? rightIt.nextInt() : Integer.MAX_VALUE;
        } else {
            rightIt = IntIterators.EMPTY_ITERATOR;
